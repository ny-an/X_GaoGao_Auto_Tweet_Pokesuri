// === Geminiプロンプト調整用 Mockテスト関数 ===
function testMockGemini() {
  // 1. テスト用のMockデータ（配列）
  const mockTweets = [
    {
      like_count: 248,
      text: "うおおーー！！ アプデ前に全員ポーズできたー！！ ビルドアーーーップ！！！！ #ポケモンスリープ",
      url: "https://x.com/do10685/status/2026140790440009802"
    },
    {
      like_count: 248,
      text: "デカいぬいを連れていくと、このようなファンサがもらいやすくなるのでオススメです🤎 #ポケパークカントー",
      url: "https://x.com/onakanouene/status/2025965596580667488"
    },
    {
      like_count: 85,
      text: "気になる方いるかもなのでうちのヘラクロスの性能貼っときます おてきのとかなら9000軽く超えると思われます 最強です #ポケモンスリープ",
      url: "https://x.com/do10685/status/2026269086330839426"
    }
  ];

  // 2. Mockデータをプロンプト用の文字列（sourcesMain）に整形
  const sourcesMain = mockTweets.map((t, i) => {
    // 実際の処理と同じように、URLを除去し120文字でカットする処理をシミュレート
    const cleanText = t.text.replace(/https?:\/\/t\.co\/[0-9A-Za-z]+/g, '').replace(/\s+/g, ' ').trim().slice(0, 120);
    return `${i + 1}. ❤️${t.like_count}\n${cleanText}\n${t.url}`;
  }).join('\n\n');

  const today = Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyy/M/d');
  const totalParts = mockTweets.length;

  Logger.log('📝 投入するMockデータ:\n' + sourcesMain);

  // 3. テスト対象のプロンプト（体言止め厳守版）
  const prompt = `
今日の日付は${today}。
...
`.trim();
  ...
  // 4. Gemini呼び出し（03_AI_Gemini.gs の関数を使用）
  Logger.log('🤖 MockデータでGeminiにリクエスト送信中...');
  const out = callGeminiAPI(prompt);
  Logger.log('✅ 生成結果:\n' + out);
}